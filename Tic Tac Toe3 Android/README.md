# Tic-Tac-Toe-Game-Android
A very simple Tic-Tac-Toe Android game that includes all the main gameplay features. It includes handling  exceptions  that may occur while playing. Built using Material Design (MD) principles with Kotlin and XML, this project was completed as a task for a session in the DEPI Android Track.

### Key Features
Core Gameplay: Complete XO logic with player turn switching.

Exception Handling: Prevents common gameplay errors, such as overwriting an occupied cell or clicking after the game ends.

UI/UX: Designed using Material Design for a clean and responsive interface.

State Management: Includes "Play Again" functionality to reset the board and a "Go Back" button for easy navigation.

### Tech Stack
Language: Kotlin

Layout: XML (ConstraintLayout)
